...
export const inviteMatch = asyncHandler(async (req, res) => {
  ...
  sendToUser(friend_id, "match_invite", {
    matchId: match.id,
    fromUserId: userId,
  })
...
export const acceptMatchInvite = asyncHandler(async (req, res) => {
  ...
  sendToUser(match.player1Id, "match_accepted", {
    matchId: updated.id,
    byUserId: userId,
  })
...
export const setBoard = asyncHandler(async (req, res) => {
  ...
  // 🔴 THIS WAS THE BUG:
  // previously it never changed status to IN_PROGRESS, so match.status stayed BOARD_SETUP forever
  if (nowHasPlayer1Board && nowHasPlayer2Board && match.status === "BOARD_SETUP") {
    updatedMatch = await prisma.match.update({
      where: { id: matchId },
      data: {
        status: "IN_PROGRESS",
        currentTurnUserId: match.player1Id, // or randomize
    ...
    broadcastToUsers(
      ...
      "board_setup_complete",
      {
        matchId: updatedMatch.id,
        startingUserId: updatedMatch.currentTurnUserId,
      },
    )
  }
...
export const getMatchState = asyncHandler(async (req, res) => {
  ...
  const opponentBoard = match.boards.find((b) => b.userId !== userId)

  const movesSet = new Set(match.moves.map((m) => m.number))

  const yourLines = yourBoard ? computeLines(yourBoard.numbers, movesSet) : 0
  const opponentLines = opponentBoard ? computeLines(opponentBoard.numbers, movesSet) : 0
...
export const makeMove = asyncHandler(async (req, res) => {
  ...
  const nextTurnUserId = opponentId

  // Update turn on match
  ...
  sendToUser(opponentId, "opponent_move", {
    matchId,
  ...
  // Notify whose turn is next
  sendToUser(nextTurnUserId, "your_turn", {
    matchId,
  })
...
export const getActiveMatchWithFriend = asyncHandler(async (req, res) => {
  const userId = req.user.id
  ...
  const match = await prisma.match.findFirst({
    where: {
      ...
      status: {
        in: ["INVITED", "BOARD_SETUP", "IN_PROGRESS"],
      },
    },
    orderBy: {
      createdAt: "desc",
  ...
  return new ApiResponse(
    res,
    200,
    { hasActiveMatch: true, matchId: match.id },
    true,
    "Active match found",
  )
})
